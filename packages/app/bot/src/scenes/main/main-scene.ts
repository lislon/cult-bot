...
const content = (ctx: ContextMessageUpdate) => {
    ...
    const maybeLk = botConfig.FEATURE_BILLING ? ['lk'] : []

    const menu = [
        ['customize'],
        ['tops', 'packs'],
        ...[(isAdmin(ctx) ? ['search', 'admin', ...maybeLk] : ['search', ...maybeLk])],
        ['feedback', 'favorites'],
    ]
...
function postStageActionsFn(bot: Composer<ContextMessageUpdate>): void {
    bot
        ...
        .hears(i18nModuleBtnName('lk'), async ctx => {
            await ctx.scene.enter('lk_scene')
        })

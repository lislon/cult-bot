...
export function buildPostgresMd5EventsExpression(prefix = ''): string {
...
export class EventsSyncRepository {
    ...
    public async updateTagsLevel2(tags: TagUpdate[], dbTx: ITask<IExtensions>): Promise<void> {
        if (tags.length > 0) {
            const s = this.pgp.helpers.update(tags.map(t => ({
                id: t.id,
                tag_level_2: t.tagLevel2
            })), this.dbColUpdateTagLevel2) + ' WHERE v.id = t.id'
            await dbTx.none(s)
        }
    }
    ...
    private static mapToDb(event: EventToSave, updatedAt: Date): DbEvent {
        return {
            ...
            address: event.primaryData.address,
            geotag: event.primaryData.geotag,
            timetable: event.primaryData.timetable,
            duration: event.primaryData.duration,
            price: event.primaryData.price,
            notes: event.primaryData.notes,
            ...
            tag_level_2: event.primaryData.tag_level_2,
            tag_level_3: event.primaryData.tag_level_3,
            rating: event.primaryData.rating,
            reviewer: event.primaryData.reviewer,
            likes: event.primaryData.likes,
            dislikes: event.primaryData.dislikes,

...
import { SubscriptionRepository } from './db-subscription'
...
const initOptions: IInitOptions<IExtensions> = {
    ...
    extend(dbEx: BotDb) {
        const extensions: IExtensions = {
            ...
            repoBilling: new BillingRepository(dbEx, pgp),
            repoSubscription: new SubscriptionRepository(dbEx, pgp)
        }
...
export function pgLogVerbose(): void {
...
export function pgLogOnlyErrors(): void {
    if (pgMonitor.isAttached()) {
        pgMonitor.detach()
    }

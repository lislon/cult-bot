...
export async function cleanDb() {
    return await db.none('TRUNCATE cb_events_entrance_times, cb_events, cb_events_packs, cb_billing_payments, cb_billing_payments_notifications RESTART identity CASCADE')
}
...
export async function givenUnpaidSubscription(userId: number): Promise<number> {
    return db.repoSubscription.createSubscription({
        createdAt: new Date(),
        periodDays: 10,
        userId: userId
    })
